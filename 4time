...
APP_KEY = "PSk9VNngO47n4S8Q2rNajtTNGyWSnbLwC0OD"
APP_SECRET = "J+z/VW20q/QbwuF1G2hKnRuC002JNPO57XtxylOLHVZKSmgbtPs7YNa68ZNpDf1RofF1XhD2ERhGLgq3HeJ5y3sOEdbbnTORNDAjofuE5Eg1+i+d0CpKcTh3aRJI8JPgwI7u3qc1s4kAfoTZZXLYI1uHaYZONPywAPmLJpWxopJkaLkHV2M="
ACCESS_TOKEN = ""
CANO = "64897987"
ACNT_PRDT_CD = "01"
DISCORD_WEBHOOK_URL = "https://discord.com/api/webhooks/1349297849774510121/1OEf1gcPEYod0zkTMsXrYsJ1ht8v1jnpshf5QVbpQoOlsESQo5Yof7-KxO0z_JUwLl02"
    ...
    while True:
        ...
        if t_9 < t_now < t_start and soldout == False: # 잔여 수량 매도
            for sym, qty in stock_dict.items():
                if trade_count >= 4:
                    send_message("하루 최대 거래 횟수(4회) 도달. 추가 매매 중지.")
                    break
                market1 = "NASD"
                market2 = "NAS"
                ...
                if sym in amex_symbol_list:
                    market1 = "AMEX"
                    market2 = "AMS"
                if sell(market=market1, code=sym, qty=qty, price=get_current_price(market=market2, code=sym)):
                    trade_count += 1  # 매도 성공 시 거래 횟수 증가
            soldout = True
            bought_list = []
            time.sleep(1)
            stock_dict = get_stock_balance()
        if t_start < t_now < t_sell and trade_count < 4:  # AM 09:35 ~ PM 03:45 : 매수 (거래 횟수 체크 추가)
            for sym in symbol_list:
                if len(bought_list) < target_buy_count and trade_count < 4:
                    ...
                    if target_price < current_price:
                        ...
                        if buy_qty > 0:
                            ...
                            if buy(market=market1, code=sym, qty=buy_qty, price=current_price):
                                trade_count += 1  # 매수 성공 시 거래 횟수 증가
                                bought_list.append(sym)
                                get_stock_balance()
                    time.sleep(1)
            time.sleep(1)
        if t_sell < t_now < t_exit:  # PM 03:45 ~ PM 03:50 : 일괄 매도
            if soldout == False:
                ...
                bought_list = []
                time.sleep(1)
        if t_exit < t_now:  # PM 03:50 ~ :프로그램 종료 및 거래 횟수 초기화
            send_message("프로그램을 종료합니다. 하루 거래 횟수를 초기화합니다.")
            trade_count = 0
            break
